...
def update_plot(n_intervals, reset_clicks):
    global x_data, temp_data, weight_data, t0
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=x_data, y=temp_data, mode="lines+markers", name="Temperature (°C)"))
    fig.update_layout(
        xaxis_title="Time (s)",
    ...
    if len(weight_data)!=0:
        if weight_data[-1]>0.6*weight_target:
            color = "yellow"
        if weight_data[-1]>0.8*weight_target:
            color = "orange"
        if weight_data[-1]>weight_target:
            color = "red"
    fig2.add_trace(go.Scatter(x=x_data, y=weight_data,line_color = color, mode="lines+markers", name="Weight (kg)"))
...
def update_table(n):
    ...
    if len(temp_data) == 0 or len(weight_data) == 0:
        current_temp = 0
        current_weight = 0
    else:
        current_temp = temp_data[-1]
        current_weight = weight_data[-1]
